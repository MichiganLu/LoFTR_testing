...
class Transformer(nn.Module):
    def __init__(self):
        super().__init__()
        self.transformer = torch.load('./temp_backbone/transformer1.pth')

    def forward(self, feat_c0, feat_c1, feat_f0, feat_f1, data):
        return self.transformer(feat_c0, feat_c1, feat_f0, feat_f1, data)

class Pruned_LoFTR(nn.Module):
    def __init__(self):
        super().__init__()
        self.backbone = Pruned_Backbone()
    ...
    def forward(self, data):
        ...
        self.transformer(feat_c0, feat_c1, feat_f0, feat_f1, data)
    ...
    def extract_feature(self, data, flag):
        ...
        if flag:  # if the first image has been input before, ignore the first image
            ...
            data.update({
                'hw0_c': feat_c1.shape[2:] * ratio, 'hw1_c': feat_c1.shape[2:],
                # always dimension is [batch, channel, height, width], so here you are only taking heght, weight of feature map
                'hw0_f': feat_f1.shape[2:] * ratio, 'hw1_f': feat_f1.shape[2:]
                # the height and width is feature map dimension
            })
            ...
            if data['hw0_i'] == data['hw1_i']:  # faster & better BN convergence
                feats_c, feats_f = self.backbone(torch.cat([data['image0'], data['image1']],
                                                           dim=0))  # feed into backbone get feature map for img0 and img1, concatenate in batch dimension. backbone returns two feature map, one for 1/2 and one for 1/8 feature map
                (feat_c0, feat_c1), (feat_f0, feat_f1) = feats_c.split(data['bs']), feats_f.split(
                    data['bs'])  # split feature map into two for two images
            else:  # handle different input shapes
            ...
            # 2. coarse-level loftr module
            # add featmap with positional encoding, then flatten it to sequence [N, HW, C]
            feat_c0 = rearrange(self.pos_encoding(feat_c0),
                                'n c h w -> n (h w) c')  # add space encoding to coarse feature map of img0, and reshape to [N, HW, C]!!!
            feat_c1 = rearrange(self.pos_encoding(feat_c1),
                                'n c h w -> n (h w) c')  # add space encoding to coarse feature map of img1, and reshape to [N, HW, C]!!!
    ...
    def transformer(self, feat_c0, feat_c1, feat_f0, feat_f1, data):
        return self.transformer(self, feat_c0, feat_c1, feat_f0, feat_f1, data)
'''
